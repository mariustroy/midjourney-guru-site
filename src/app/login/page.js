...
export default function Login() {
  ...
  async function sendCode(e) {
    ...
    const { error } = await supa.auth.signInWithOtp({
      email  : addr,
      options: { shouldCreateUser: true }   // numeric OTP, no redirect
    });
  ...
  /* ---------- verify helper (uses token param) ---------- */
  async function verifyToken(token) {
    const { error } = await supa.auth.verifyOtp({
      email: email.trim().toLowerCase(),
      token,
      type : "email"                     // channel for 6-digit codes
    });

    if (error) { setErr(error.message); return; }
    router.replace("/");                 // success → dash
  }
  ...
  return (
    <main className="relative isolate min-h-screen flex flex-col bg-black text-[var(--brand)]">
      {/* bg */}
      <Image src="/images/hero.jpg" alt="" fill unoptimized priority
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-xs mx-auto space-y-4">
            <input
              ...
              onChange={e => {
                ...
                setCode(digits);
                if (digits.length === 6) verifyToken(digits);   // 🚀 auto-submit
              }}
